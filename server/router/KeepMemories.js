...
const detailPagesKeepMemories = 
    {
        ...
        projectIntroExp: '개인프로젝트를 하게 된 계기는 명확했습니다. 팀 프로젝트를 통해 부족한 점이 많았기 때문입니다. react에 대한 이해도 그리고 매우 중요한 데이터 상태관리 등 스스로 처음부터 끝까지 웹사이트를 만들며 이해하고 경험하고 해결하는 과정을 얻고 싶었습니다. 스스로 웹사이트를 구성하면서 전반적으로 많은 지식을 습득하고 이해하고 에러를 발견 할 시 해결하는 집념, 근성도 성장하는 계기가 되었습니다. 미숙한 점도 많지만 스스로 만들어낸 첫 웹사이트인 만큼 저에겐 의미가 큰 것 같습니다.',
        typeScriptMainTitle: 'TypeScript를 사용한 이유',
        typeScriptTitleOne : '높은 수준의 코드 탐색과 디버깅',
        typeScriptTitleOneIntro : '타입스크립트는 코드에 목적을 명시하고 목적에 맞지 않는 타입의 변수나 함수들에서 에러를 발생시켜 버그를 사전에 제거합니다. 또한 코드 자동완성이나 실행 전 피드백을 제공하여 작업과 동시에 디버깅이 가능해 생산성을 높일 수 있습니다.',

        typeScriptTitleTwo : '자바스크립트 호환',
        typeScriptTitleTwoIntro : '타입스크립트는 자바스크립트와 100% 호환됩니다. 따라서 프론트엔드 또는 백엔드 어디든 자바스크립트를 사용할 수 있는 곳이라면 타입스크립트도 쓸 수 있습니다. 타입스크립트는 앱과 웹을 구현하는 자바스크립트와 동일한 용도로 사용 가능하며 서버 단에서 개발이 이루어지는 복잡한 대형 프로젝트에서도 빛을 발합니다.',
        
        typeScriptTitleThr : '강력한 생태계',
        typeScriptTitleThrIntro : ' 타입스크립트는 그리 오래되지 않은 언어임에도 불구하고 강력한 생태계를 가지고 있습니다. 대부분의 라이브러리들이 타입스크립트를 지원하며 마이크로소프트의 비주얼 스튜디오 코드(VSCode)를 비롯해 각종 에디터가 타입스크립트 관련 기능과 플러그인을 지원합니다.',
        
        reduxTitle : 'Redux',
        reduxIntro : 'Props 문법의 번거로움과 복잡 다양한 상태의 효율적인 관리를 위해 상태 관리 라이브러리 Redux를 사용하게 되었습니다. 앞에서 말한 두가지 장점을 구체적으로 설명하면 기존 React 라이브러리만 사용하는 경우에는 부모에서 자식의 자식의 자식까지 상태를 내렸어야 했는데, 리덕스를 사용하면 스토어를 사용하여 상태를 컴포넌트 구조의 바깥에 두고, 여러 컴포넌트를 거쳐서 받아올 필요 없이 아무리 깊숙한 컴포넌트에 있다 하더라도 직속 부모에게서 받아오는 것 처럼 원하는 상태값을 골라서 props 를 편리하게 받아올 수 있습니다. 또한 스토어를 통해 상태를 한 곳에서 관리하기 때문에 유지 보수가 상대적으로 용이합니다. . 따라서 데이터를 추가하고 삭제하고 해당하는 데이터에 글을 기록하고 삭제하는 모든 과정을 Redux를 통하여 관리하였습니다.',
        
        functionIntroTitleOne : '메인 페이지 및 데이터 삭제',
        ...
        functionIntroTitleFour : '즐겨찾기 목록 삭제 및 모달창(글 작성 및 삭제)',
        functionIntroFour : ' 사이트를 구성하면서 즐겨찾기한 데이터의 세부 정보를 보려면 어떤 방식으로 해야 할까하는 고민을 하였습니다. 이 부분을 앞서 4인 프로젝트에서 경험했던 모달창으로 구현 하였습니다. 모달창을 클릭시에 해당 데이터를 확인 할 수 있게 구현하였고 또한 글 기록할 수 있도록 구현 하였습니다. 모달창 데이터는 데이터를 서버에 호출하여 받아 오는 것이 아니라 이미 데이터는 서버를 통해 받아온 상태이기 때문에 클릭한 이미지에 대한 id값을 React Props로 내려 받는 방식으로 구현하였습니다.',
