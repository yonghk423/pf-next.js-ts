...
export default function KeepMemories() {
    return(
        <div className={styles.container}>
            <div>TypeScript를 사용한 이유</div>
            <div>높은 수준의 코드 탐색과 디버깅</div>
            <div>타입스크립트는 코드에 목적을 명시하고 목적에 맞지 않는 타입의 변수나 함수들에서 에러를 발생시켜 버그를 사전에 제거합니다. 또한 코드 자동완성이나 실행 전 피드백을 제공하여 작업과 동시에 디버깅이 가능해 생산성을 높일 수 있습니다. 실제로 한 연구에 따르면 모든 자바스크립트 버그의 15%가 사전에 타입스크립트로 감지할 수 있다고 합니다.
            </div>
            <div>자바스크립트 호환</div>
            <div>타입스크립트는 자바스크립트와 100% 호환됩니다. 따라서 프론트엔드 또는 백엔드 어디든 자바스크립트를 사용할 수 있는 곳이라면 타입스크립트도 쓸 수 있습니다. 타입스크립트는 앱과 웹을 구현하는 자바스크립트와 동일한 용도로 사용 가능하며 서버 단에서 개발이 이루어지는 복잡한 대형 프로젝트에서도 빛을 발합니다.              
            </div>
            <div>강력한 생태계</div>
            <div>
            ...
            <div>사진 세부 정보 확인, 즐겨찾기 추가, 글 작성 및 삭제</div>
            <div>
            ...
            <div>즐겨찾기 목록 삭제 및 모달창(글 작성 및 삭제)</div>
            <div>
            ...
            <div>React.memo를 사용하여 컴포넌트 성능 최적화</div>
            <div>
